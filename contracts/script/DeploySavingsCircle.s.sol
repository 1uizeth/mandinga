...
///         and SavingsCircle (VRF v2.5) to Base Sepolia or Base Mainnet.
/// @dev Loads addresses from config/base-sepolia.json (or CONFIG env). Uses vm.computeCreateAddress
///      to break circular dependencies. SavingsCircle uses Chainlink VRF v2.5 with subscriptionId.
///
/// Usage:
///   forge script script/DeploySavingsCircle.s.sol --broadcast --rpc-url $RPC_URL
///   CONFIG=config/base-mainnet.json forge script script/DeploySavingsCircle.s.sol --broadcast --rpc-url $RPC_URL
contract DeploySavingsCircle is Script {
    using stdJson for string;

    function _loadConfig() internal view returns (DeployConfig memory c) {
        string memory configPath = vm.envOr("CONFIG", string("config/base-sepolia.json"));
        ...
        c.subscriptionId = subId != 0 ? subId : configJson.readUint(".chainlink.subscriptionId");
    }
